...
const PokemonCollection: FC = () => {
  ...
  const loadNextPage = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get(
        nextUrl || `${pokeapiGet}?limit=${NUM_POKE_LOAD}&offset=${loadTimes * NUM_POKE_LOAD}`
      );
  ...
  return (
    <section ref={thisCollection} className='flexcentercol flex-wrap text-ctbackground font-semibold mt-[4rem]'>
      ...
      {pokemons.length ? (
        <PokemonPagination localPageIdx={localPageIdx} pokemons={pokemons} setPageIdx={setPageIdx} />
      ) : (
        ''
      )}
