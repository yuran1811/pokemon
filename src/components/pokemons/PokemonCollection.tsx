...
const PokemonCollection: FC = () => {
  const [localData, setLocalData] = useLocalStore<Pokemon[]>(
    "pokemons",
    [],
    "[]",
  );
  const [localPageIdx, setLocalPageIdx] = useLocalStore<number>(
    "pageIdx",
    0,
    "0",
  );
  const [localLastSearch, setLocalLastSearch] = useLocalStore<string>(
    "lastSearch",
    "null",
    "null",
  );
  ...
  const [listPokes, setListPokes] = useState<listPokesType>({
    pokemons: localData,
    change: 0,
  });
  const [loadTimes, setLoadTimes] = useState<number>(
    localData.length / NUM_POKE_LOAD,
  );
  const [pokesEachPage, setPokesEachPage] = useState<Pokemon[]>(
    pokemons.slice(0, pageIdx * NUM_POKE_LOAD),
  );
  ...
  const [listPokesSearch, setListPokesSearch] = useReducer(
    (
      listPokesSearch: stateReducerType,
      { searchValue }: PokemonAction,
    ): stateReducerType => {
      ...
      return {
        searching: true,
        searchValue,
        pokemons: pokemons.filter((_) => _.name.includes(searchValue)),
      };
    },
    {
      searching: false,
      searchValue: localLastSearch === "null" ? "" : localLastSearch,
      pokemons,
    },
  );
  ...
  const loadNextPage = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get(
        nextUrl ||
          `${pokeapiGet}?limit=${NUM_POKE_LOAD}&offset=${loadTimes * NUM_POKE_LOAD}`,
      );
  ...
  useEffect(() => {
    setListPokes(({ change }) => ({
      pokemons: listPokesSearch.searching
        ? listPokesSearch.pokemons
        : pokesEachPage,
      change: change + 1,
    }));
  ...
  useEffect(() => {
    setPokesEachPage(() =>
      pokemons.slice((pageIdx - 1) * NUM_POKE_LOAD, pageIdx * NUM_POKE_LOAD),
    );
  ...
  return (
    <section
      ref={thisCollection}
      className="flexcentercol text-ctbackground container mx-auto mt-16 flex-wrap font-semibold"
    >
      <PokemonSearch
        value={localLastSearch}
        placeholder="Type to search"
        setListPokesSearch={setListPokesSearch}
      />

      {viewDetail.isOpened && viewDetail.id && (
        <Overlay
          zIdx="z-11"
          onClick={() => setDetail({ id: 0, isOpened: !viewDetail.isOpened })}
        />
      )}
